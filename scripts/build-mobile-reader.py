...
Usage:
    .venv/bin/python scripts/build-mobile-reader.py
...
def extract_main_styles(main_html: str) -> str:
    """Return all <style> blocks from the main shell, concatenated.
    ...
    grids) as the main app. Keeping the blocks separate would also work but
    smashing them into one keeps the head smaller and easier to review.
    """
    blocks = _STYLE_BLOCK_RE.findall(main_html)
    if not blocks:
        raise RuntimeError("No <style> blocks found in candlestick-patterns.html")
    return "\n\n/* ──────── next inlined block ──────── */\n\n".join(blocks)
