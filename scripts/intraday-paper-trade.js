...
// itpPaperTradeTick window.* exposures stay global for the inline handlers in
...
(function intradayPaperTradeModule() {
  var STORAGE_KEY = 'intraday_paper_trade_state_v1';
  ...
  // We MUST declare this here because intradayPaperTradeModule is its own IIFE
  ...
  console.log('[intraday-paper-trade] init: BASE =', BASE);
  ...
  var PT_API_PAUSED_KEY = 'itp_api_paused_v1';
  ...
  // The intraday tab has its OWN Live/Paused master switch (window.itIsApiPaused,
  // owned by scripts/intraday-trade.js). This clone defers to it so the single
  // header toggle governs the chart, cards, setup engine AND this paper book —
  // no second hidden switch. Falls back to its own key only if the intraday
  // module hasn't loaded yet (defensive; both default to PAUSED = no live HTTP).
  function itpIsApiPaused() {
    try {
      if (typeof window.itIsApiPaused === 'function') return !!window.itIsApiPaused();
      return localStorage.getItem(PT_API_PAUSED_KEY) !== '0';
    } catch (_) { return true; }
  }

  function itpSetApiPaused(flag) {
    try { localStorage.setItem(PT_API_PAUSED_KEY, flag ? '1' : '0'); }
    catch (_) {}
  }

  // Exposed on window so the chain module can check it too.
  window.itpIsApiPaused = itpIsApiPaused;

  // Market-hours — prefer the intraday tab's OWN helper (window.itIsMarketOpen,
  // owned by scripts/intraday-trade.js, which has a real IST fallback), then the
  // shared holiday-aware impl, then a local weekday/9:15-15:30 IST fallback.
  // Never assumes "always open" — fail-safe per the trading rules.
  function itpMarketOpen() {
    try {
    ...
    var ist = new Date(Date.now() + (5 * 3600 + 30 * 60) * 1000);
  ...
  var state = {
    ...
    // calling itpBuyCE / itpBuyPE. They're consumed once on the
    ...
    // NOTE: the Options-tab "Live Option Chain table piggyback"
    // (chainTableLtpKeys) was removed — the intraday book never feeds the
    // Options/Live section's chain table.
  ...
  // ─── Server-JSON persistence (survives a browser storage clear) ──────────
  // The book is mirrored to data/intraday-paper-trades.json on disk via the
  // dev server (server.py: GET/POST /local/intraday-trades). localStorage is
  // the fast first-paint store; the server file is the durable source of truth
  // that a "clear cache / storage" cannot wipe. On a deployed static PWA (no
  // server) the POST/GET simply fail and we degrade to localStorage-only.
  var SERVER_TRADES_URL = '/local/intraday-trades';
  var serverSaveTimer = null;
  var serverHydrated = false;

  // Whether the server JSON mirror is reachable. Only same-origin localhost /
  // 127.* / *.local dev runs have server.py; anywhere else we skip the round
  // trip entirely (avoids a 404/blocked fetch on every save in production).
  function serverPersistAvailable() {
    try {
      var h = (location.hostname || '').toLowerCase();
  ...
  // Human-friendly closed-trade list with EXACTLY the Trade-History columns the
  // user asked to persist (Result / Instrument / Side / Lots / Entry / Exit /
  // Pts / P&L / Held). Derived from state.history so the on-disk JSON is
  // self-describing, not just an opaque state blob.
  function historyAsTrades() {
    return (state.history || []).map(function (h) {
      var lots = (typeof h.qty === 'number') ? h.qty : null;
      var instrument = (h.kind === 'OPT')
        ? (h.strike + ' ' + h.optType)
        : (h.symbol || 'NIFTY');
      var side = (h.kind === 'OPT') ? ('BUY ' + h.optType) : (h.side || '');
      var held = (h.entryTs && h.exitTs) ? fmtDur(h.exitTs - h.entryTs) : '';
      return {
        result: h.exitReason || '',                 // MANUAL / SL / TGT / EOD
        instrument: instrument,
        side: side,
        lots: lots,
        entry: (typeof h.entry === 'number') ? +h.entry.toFixed(2) : null,
        exit: (typeof h.exit === 'number') ? +h.exit.toFixed(2) : null,
        pts: (typeof h.pts === 'number') ? +h.pts.toFixed(2) : null,
        pnl: (typeof h.pnl === 'number') ? Math.round(h.pnl) : null,
        held: held,
        entryTs: h.entryTs || null,
        exitTs: h.exitTs || null
      };
    });
  }
  ...
  function serverPayload() {
    return {
      ...
      qtyCE: state.qtyCE,
      qtyPE: state.qtyPE,
      trades: historyAsTrades(),        // the requested column list
      savedAt: new Date().toISOString()
    };
  }

  // Debounced write-through to the server file (250ms): rapid edits (e.g. EOD
  // squaring off several positions) collapse into one disk write.
  function scheduleServerSave() {
    if (!serverPersistAvailable()) return;
    if (serverSaveTimer) clearTimeout(serverSaveTimer);
    serverSaveTimer = setTimeout(function () {
      serverSaveTimer = null;
      try {
        fetch(SERVER_TRADES_URL, {
          method: 'POST',
  ...
  // One-time hydrate from the server file on tab open. If the file holds a book
  // (it survived a storage clear), adopt it as the source of truth, mirror to
  // localStorage, and re-render. Never throws — a missing/unreachable server
  // just leaves the localStorage-loaded book in place.
  function hydrateFromServer(done) {
    if (serverHydrated || !serverPersistAvailable()) { if (done) done(); return; }
    serverHydrated = true;
    try {
      fetch(SERVER_TRADES_URL, { credentials: 'omit', cache: 'no-store' })
        .then(function (r) { return r.ok ? r.json() : null; })
        .then(function (s) {
          if (s && typeof s === 'object' && (Array.isArray(s.open) || Array.isArray(s.history))) {
            ...
            save();            // mirror back into localStorage
  ...
  function save() {
    ...
    // Durable mirror to disk (dev server only) so a storage clear can't lose it.
    scheduleServerSave();
  }
  ...
  function place(side) {
    ...
    if (!ltp) { alertModal('Chart not live', 'Live chart not running yet.\n\nConnect your Upstox token and flip the intraday switch to LIVE first.'); return; }
    state.open.push({
  ...
  async function placeOpt(optType) {
    if (itpIsApiPaused()) {
      alertModal('Intraday API paused', 'Flip the intraday header switch to LIVE to place trades.');
      return;
    }
    var chain = window.itOptionChainData;
    if (!chain) {
      alertModal('No option chain', 'Option chain not loaded yet. Flip the intraday switch to LIVE — strikes load automatically once connected, or tap REFRESH.');
      return;
    }
    if (state.selectedStrike == null) {
      alertModal('Pick a strike', 'Select a strike first.\n\nClick the strike picker to choose one.');
      return;
    }
    var strike = state.selectedStrike;
    var row = chain.strikes.filter(function (s) { return s.strike_price === strike; })[0];
    if (!row) { alertModal('Strike not found', 'That strike is not in the current chain. Tap REFRESH to reload it.'); return; }
    var leg = optType === 'CE' ? row.call_options : row.put_options;
    if (!leg || !leg.market_data) { alertModal('No data', 'No data available for ' + optType + ' at ' + strike + '.'); return; }
    var cachedPremium = +leg.market_data.ltp;
    if (!isFinite(cachedPremium) || cachedPremium <= 0) {
      alertModal('Premium unavailable', 'Premium for ' + strike + ' ' + optType + ' is zero / not available right now.\n\nTry a different strike or tap REFRESH to update prices.');
      return;
    }
    ...
    if (orderType === 'LIMIT' || orderType === 'STOP') {
      ...
      console.log('[intraday-paper-trade] PENDING placed', {
    ...
    var ceBtn = $('itp-buy-ce-btn'), peBtn = $('itp-buy-pe-btn');
    ...
    try {
      ...
      console.log('[intraday-paper-trade] BUY entry resolved', {
      ...
      state.open.push({
        ...
        // Anchor the Nifty spot at entry for EVERY buy (not just
        // engine-driven ones). Manual buys have no engine prime, so
        // fall back to the last live spot tick. This anchor powers the
        // premium-to-spot mapping shown under the SL/Target inputs —
        // it tells the trader which Nifty level a given premium trigger
        // corresponds to. Null only if we've never seen a spot tick.
        entrySpot: engineEntrySpot
          || (isFinite(state.lastLTP) && state.lastLTP > 0 ? state.lastLTP : null)
      });
      ...
      console.error('[intraday-paper-trade] BUY threw — restoring buttons', e && e.message);
      if (activeBtn && prevText) activeBtn.textContent = prevText;
      if (ceBtn) ceBtn.disabled = false;
      if (peBtn) peBtn.disabled = false;
      alertModal('Trade failed', 'Could not place the trade right now: ' + (e && e.message ? e.message : 'unknown error') + '\n\nPlease retry.');
    }
  }
  ...
  async function exit(id, reason, exitPxOverride) {
    ...
    console.log('[intraday-paper-trade] EXIT result', {
    ...
    if (px != null && px === pos.entry && priceSource === 'cached-fallback') {
      console.warn('[intraday-paper-trade] FLAT trade with cached-fallback price — LTP poll likely failing. Check earlier [intraday-paper-trade] warnings in console.');
    }

    if (!px) {
      if (!reason || reason === 'MANUAL') {
        alertModal('No live price', 'Live price not available yet for this position. Wait for the next tick.');
      }
      return;
    }
  ...
  // Called from every itpPaperTradeTick(ltp), i.e. every Nifty spot
  ...
  function checkSpotRiskTriggers(spot) {
    ...
    state.open.slice().forEach(function (p) {
      ...
      console.log('[intraday-paper-trade] SPOT trigger fired', {
  ...
  function checkRiskTriggers() {
    ...
    state.open.slice().forEach(function (p) {
      ...
      if (p.sl != null || p.tgt != null) {
        if (!isFinite(px) || px <= 0) {
          console.warn('[intraday-paper-trade] checkRiskTriggers: skipping ' + p.strike + p.optType
            + ' — no live price in cache (sl=' + p.sl + ', tgt=' + p.tgt + ')');
        } else {
          console.log('[intraday-paper-trade] checkRiskTriggers: ' + p.strike + p.optType
  ...
  function checkPendingTriggers() {
    ...
    state.pending.slice().forEach(function (p) {
      ...
      anyFilled = true;
      console.log('[intraday-paper-trade] PENDING filled', {
  ...
  function cancelPending(id) {
    ...
    console.log('[intraday-paper-trade] PENDING cancelled', {
  ...
  function doBulkCancel(silent) {
    var n = state.pending.length;
    state.pending = [];
    console.log('[intraday-paper-trade] PENDING bulk-cancelled', { count: n, silent: !!silent });
    if (!silent) {
  ...
  function confirmModal(opts) {
    opts = opts || {};
    var isAlert = !!opts.alert; // single-button acknowledge (replaces native alert)
    return new Promise(function (resolve) {
      var modal = document.getElementById('pt-confirm-modal');
      if (!modal) {
        // Themed modal markup absent (should never happen — it lives in the
        // shell). Degrade to native only as a last resort.
        if (isAlert) { window.alert(opts.message || opts.title || ''); resolve(true); }
      ...
      titleEl.textContent = opts.title || (isAlert ? 'Heads up' : 'Are you sure?');
      okBtn.textContent     = opts.confirmLabel || (isAlert ? 'OK' : 'CONFIRM');
      cancelBtn.textContent = opts.cancelLabel  || 'CANCEL';
      // Alert mode = acknowledge only → hide Cancel (restored for confirm).
      ...
      setTimeout(function () {
        ((danger && !isAlert) ? cancelBtn : okBtn).focus();
      }, 30);
    });
  }

  // Themed replacement for window.alert — single OK button, returns a Promise
  // that resolves when acknowledged. Use instead of native alert() so every
  // intraday popup matches the app theme (dark/light) instead of the OS dialog.
  function alertModal(title, message) {
    return confirmModal({ alert: true, title: title, message: message });
  }
  // Exposed so scripts/intraday-trade.js (and any intraday code) can raise
  // app-themed dialogs without its own modal markup.
  window.itpAlert = function (title, message) { return alertModal(title, message); };
  window.itpConfirm = function (opts) { return confirmModal(opts || {}); };
  ...
  // Read the live signed delta for a position's strike/leg from the
  // intraday option chain (CE delta positive, PE delta negative).
  // Returns null when the chain isn't loaded or the strike/leg is
  // missing — callers fail safe (hide the spot hint).
  function posLegDelta(p) {
    if (!p || p.kind !== 'OPT') return null;
    var chain = window.itOptionChainData;
    if (!chain || !Array.isArray(chain.strikes)) return null;
    for (var i = 0; i < chain.strikes.length; i++) {
      var s = chain.strikes[i];
      if (!s || s.strike_price !== p.strike) continue;
      var leg = (p.optType === 'CE') ? s.call_options : s.put_options;
      var g = leg && leg.option_greeks ? leg.option_greeks : null;
      var d = (g && typeof g.delta === 'number' && isFinite(g.delta)) ? g.delta : null;
      return d;
    }
    return null;
  }

  // Map a PREMIUM trigger (the value the user typed in the SL / Target
  // box) back to the NIFTY SPOT level it corresponds to. The reverse of
  // the recommendation card's spot-to-premium map: premium moves about
  // delta x index-points, so spot ~= entrySpot + (prem - entryPrem) / delta.
  // Signed delta makes this correct for both legs — a CE SL maps to a
  // LOWER spot, a PE SL maps to a HIGHER spot. Returns null (hide the
  // hint) when the chain isn't loaded, the entry spot wasn't captured,
  // or delta is too small to invert safely. Approximate — ignores theta
  // / gamma / IV drift, which is fine for a sub-30-min scalp.
  function premiumToSpot(p, prem) {
    if (!p || p.kind !== 'OPT' || !isFinite(prem)) return null;
    if (!isFinite(p.entrySpot) || p.entrySpot <= 0) return null;
    if (!isFinite(p.entry) || p.entry <= 0) return null;
    var d = posLegDelta(p);
    if (d == null || Math.abs(d) < 0.05) return null;
    var spot = p.entrySpot + (prem - p.entry) / d;
    if (!isFinite(spot) || spot <= 0) return null;
    return spot;
  }

  // Render the small "approx Nifty XXXXX" hint shown under an SL/Target
  // premium input. Empty string when not computable so the cell stays
  // clean rather than showing a misleading placeholder.
  function spotHintHtml(p, prem) {
    var spot = premiumToSpot(p, prem);
    if (spot == null) return '';
    return '<div class="pt-risk-spot" title="The approximate Nifty spot level this premium maps to '
      + '(via delta). Exit when the index reaches here.">\u2248 Nifty '
      + Math.round(spot) + '</div>';
  }

  function renderOpen() {
    var tb = $('itp-open-tbody');
    if (!tb) return;
      ...
      tb.innerHTML = state.open.map(function (p) {
        ...
        if (p.kind === 'OPT') {
          ...
          slCell = '<td class="pt-c-sl">'
            ...
            +   'onchange="itpUpdatePosRisk(\'' + p.id + '\',\'sl\',this.value)" '
            +   'onclick="this.select()">'
            + ((p.sl != null) ? spotHintHtml(p, p.sl) : '')
            + '</td>';
          tgtCell = '<td class="pt-c-tgt">'
            ...
            +   'onchange="itpUpdatePosRisk(\'' + p.id + '\',\'tgt\',this.value)" '
            +   'onclick="this.select()">'
            + ((p.tgt != null) ? spotHintHtml(p, p.tgt) : '')
            + '</td>';
        } else {
        ...
        return '<tr data-pos-id="' + p.id + '">'
          ...
          + slCell
          + tgtCell
          + '<td><button class="pt-exit-btn" type="button" onclick="itpExit(\'' + p.id + '\')">EXIT</button></td>'
          + '</tr>';
      }).join('');
    }
    $('itp-open-cnt').textContent = '(' + state.open.length + ')';
  }
  ...
  function renderPending() {
    var tb = $('itp-pending-cards');
    var cnt = $('itp-pending-cnt');
    if (!tb) return;
    ...
    tb.innerHTML = state.pending.map(function (p) {
      ...
      return '<div class="pt-pend-card ' + typeCls + '" data-pending-id="' + p.id + '">'
        ...
        +   slChip + tgtChip
        +   '<button class="pt-cancel-btn" type="button" onclick="itpCancelPending(\''
        +     p.id + '\')">CANCEL</button>'
        + '</div>'
        + '</div>';
    }).join('');
  ...
  function renderStats() {
    ...
    if (!n) {
      ...
      var elN0 = $('itp-stat-n');     if (elN0) elN0.textContent = '0';
      var elWr0 = $('itp-stat-wr');   if (elWr0) elWr0.innerHTML = '\u2014';
      var elAvg0 = $('itp-stat-avg'); if (elAvg0) elAvg0.innerHTML = '\u2014';
      var elBest0 = $('itp-stat-best'); if (elBest0) elBest0.innerHTML = '\u2014';
      var elWorst0 = $('itp-stat-worst'); if (elWorst0) elWorst0.innerHTML = '\u2014';
      return;
    }
  ...
  function renderTodayPnL() {
    var el = $('itp-pnl-today');
    var sub = $('itp-pnl-today-sub');
    if (!el || !sub) return;
  ...
  function renderEquity() {
    ...
    // Open P&L stat cell
    var pnlEl = $('itp-pnl-open');
    var pnlPctEl = $('itp-pnl-open-pct');
    if (pnlEl) {
  ...
  function renderSpot() {
    ...
    var sub = $('itp-spot-sub');
    if (sub) {
      var marketOpen = itpMarketOpen();
  ...
  function setBtnState() {
    ...
    var b = $('itp-buy-btn'), s = $('itp-sell-btn');
    ...
    var ceBtn = $('itp-buy-ce-btn'), peBtn = $('itp-buy-pe-btn');
    var trigger = $('itp-strike-trigger');
    var hasChain = !!window.itOptionChainData;
  ...
  function setOrderType(sideLower, orderType) {
    ...
    var trig = $('itp-ot-trigger-' + sideLower);
    var risk = $('itp-ot-risk-' + sideLower);
    var hint = $('itp-ot-hint-' + sideLower);
  ...
  function refreshOrderHint(sideLower) {
    ...
    // chain.market_data.ltp (refreshed every 30s by window.itFetchChain).
  ...
  function resetOrderTicket(sideLower) {
    var trig = $('itp-ot-trigger-' + sideLower);
    var slEl = $('itp-ot-sl-' + sideLower);
    var tgtEl = $('itp-ot-tgt-' + sideLower);
    if (trig) trig.value = '';
    if (slEl) slEl.value = '';
  ...
  function applySelectedStrike() {
    ...
    var trigMonEl = $('itp-strike-atm-tag');
    var ceEl = $('itp-opt-ce-val'), peEl = $('itp-opt-pe-val');
    var ceMonEl = $('itp-opt-ce-money'), peMonEl = $('itp-opt-pe-money');
    // Greek pills (Delta / IV / OI). Same lookups for CE + PE sides.
    var ceDeltaEl = $('itp-ce-delta'), ceIvEl = $('itp-ce-iv'), ceOiEl = $('itp-ce-oi');
    var peDeltaEl = $('itp-pe-delta'), peIvEl = $('itp-pe-iv'), peOiEl = $('itp-pe-oi');
  ...
  function selectStrike(k) {
    ...
    startOptionPolling();
    pollOptionPrices();
    // Notify the intraday SETUP engine (scripts/intraday-trade.js) so it can
    // re-anchor SL/T1/T2/T3 to THIS strike's premium if it wants to. Calls the
    // intraday-owned hook only — NOT the old Options-tab analyzer's
    // window.intradayOnStrikeChange. No-op until intraday-trade.js defines it.
    try {
      if (typeof window.itOnStrikeChange === 'function') {
        window.itOnStrikeChange(state.selectedStrike);
      }
    } catch (_) {}
  }
  ...
  function openStrikes() {
    var pick = $('itp-strike-picker');
    var pop = $('itp-strike-pop');
    var search = $('itp-strike-search');
    if (!pick || !pop) return;
    pop.hidden = false;
  ...
  function refreshLiveAgoLabel() {
    var tag = $('itp-live-tag');
    var ago = $('itp-live-ago');
    if (!tag || !ago) return;
    if (!lastChainFetch) {
      tag.hidden = true;
      return;
  ...
  // Called by intraday-trade.js's fetchChainDirect (via window.itpOnChainLoaded)
  // after the intraday option chain is fetched — NOT from the Options tab's
  // liveChainModule.
  function onChainLoaded() {
    ...
    applySelectedStrike();
    renderStrikeList();
    // Delta is now available for held positions → re-render the open
    // table so the premium-to-spot hints under each SL/Target input
    // populate (they were blank while the chain was still loading).
    renderOpen();
  ...
  async function pollOptionPrices() {
    ...
    // Intraday SETUP engine's recommended ATM strike — include its CE+PE
    // legs so the live plan-card pricing (entry/SL/TGT cells) refreshes
    // every 2s like everything else. Uses the intraday-owned hook only
    // (window.itGetAtmInstrumentKeys), NOT the old Options-tab analyzer's
    // window.intradayGetAtmInstrumentKeys. No-op until intraday-trade.js
    // defines it (the selected-strike legs above already drive the cards).
    if (typeof window.itGetAtmInstrumentKeys === 'function') {
      try {
        var iaKeys = window.itGetAtmInstrumentKeys();
        for (var iak = 0; iak < iaKeys.length; iak++) keys.push(iaKeys[iak]);
      } catch (_) { /* non-fatal */ }
    }
    // NOTE: the Options-tab "Live Option Chain table" key-polling block was
    // removed. It keyed off document.getElementById('live') (the Options/Live
    // section) — irrelevant to the self-contained intraday book.
    ...
    var timer = ctrl ? setTimeout(function () {
      ...
      console.warn('[intraday-paper-trade] option LTP poll timed out (3s) — aborting and waiting for next tick');
    }, 3000) : null;
    try {
      ...
      if (!r.ok) {
        console.warn('[intraday-paper-trade] option LTP poll failed', r.status, await r.text().catch(function () { return ''; }));
        return;
      }
      var d = await r.json();
      if (!d || !d.data) {
        console.warn('[intraday-paper-trade] option LTP poll returned empty data');
        return;
      }
      ...
      if (matchedCount === 0) {
        console.warn('[intraday-paper-trade] poll: 0 of ' + keys.length + ' positions matched. keys=', keys, ' raw response=', d.data, ' unmatched recs=', unmatched);
      } else if (state.lastMatchedCount !== matchedCount) {
        console.log('[intraday-paper-trade] poll: matched ' + matchedCount + '/' + keys.length + ' positions');
      ...
      if (e && e.name === 'AbortError') {
        ...
        console.warn('[intraday-paper-trade] option LTP poll threw', e && e.message);
      }
    } finally {
  ...
  async function fetchFreshOptionPrice(instrumentKey, caller) {
    if (itpIsApiPaused()) return null;
    var tag = '[intraday-paper-trade] ' + (caller || 'FRESH') + ':';
    if (!instrumentKey) {
  ...
  function startOptionPolling() {
    ...
    console.log('[intraday-paper-trade] startOptionPolling — 2s cadence');
    state.lastPollSuccessTs = Date.now();
    state.pollInFlight = false;
    ...
    state.optPollWatchdog = setInterval(function () {
      ...
      if (since > WATCHDOG_STALL_MS) {
        console.warn('[intraday-paper-trade] watchdog: no successful poll in ' + since + 'ms — restarting timer');
  ...
  function shouldPollOptions() {
    if (itpIsApiPaused()) return false;
    var marketOpen = itpMarketOpen();
    ...
    if (window.itOptionChainData && state.selectedStrike != null) return true;
    // Intraday SETUP engine has an active BUY recommendation — keep polling
    // so its plan-card entry/SL/target cells re-price live. Uses the
    // intraday-owned hook only (window.itGetAtmInstrumentKeys), NOT the old
    // Options-tab analyzer's window.intradayGetAtmInstrumentKeys.
    if (typeof window.itGetAtmInstrumentKeys === 'function') {
      try {
        var iaKeys = window.itGetAtmInstrumentKeys();
  ...
  function tick(ltp) {
    ...
    try { checkSpotRiskTriggers(ltp); } catch (e) {
      console.error('[intraday-paper-trade] checkSpotRiskTriggers threw:', e && e.message);
    }
    // Push spot to the intraday SETUP engine (scripts/intraday-trade.js) so it
    // can re-price the plan card and run its spot-invalidation safety check.
    // Calls the intraday-owned hook only — NOT the old Options-tab analyzer's
    // window.intradayLiveTick. Note: intraday-trade.js's own pollTick already
    // drives its setup/journal with the same spot, so this is a belt-and-brace
    // hook that's a no-op until that module defines window.itLiveTick.
    if (typeof window.itLiveTick === 'function') {
      try { window.itLiveTick(ltp); } catch (_) {}
    }
  }

  // Convenience: trigger the option-chain fetch from inside the intraday
  // paper-trade panel. Uses the intraday tab's OWN fetch (window.itFetchChain,
  // owned by scripts/intraday-trade.js) — NOT the Options tab's upFetchChain.
  // Gives clear feedback instead of a stuck "REFRESHING..." when the fetch
  // can't run (API paused / no token / market closed with no cached chain).
  function loadStrikes() {
    var btn = $('itp-load-strikes-btn');
    var restore = function (txt) {
      if (!btn) return;
      btn.disabled = false;
      btn.innerHTML = txt || '&#8635; REFRESH';
    };
    // Pre-flight: surface the real blocker so the dropdown never hangs.
    if (itpIsApiPaused()) {
      restore('&#8635; REFRESH');
      alertModal('Intraday API paused', 'Flip the intraday header switch to LIVE to load the option chain.');
      return;
    }
    var token = '';
    try { token = (localStorage.getItem('upstox_token') || '').trim(); } catch (_) {}
    if (!token) {
      restore('&#8635; REFRESH');
      alertModal('No Upstox token', 'Add a token in API Setup to load the option chain.');
      return;
    }
    if (typeof window.itFetchChain !== 'function') {
      restore('&#8635; REFRESH');
      alertModal('Loader not ready', 'Intraday chain loader not ready. Reload the page.');
      return;
    }
    if (btn) {
      btn.disabled = true;
    ...
    var done = false;
    var prev = window.itpOnChainLoaded;
    window.itpOnChainLoaded = function () {
      done = true;
      window.itpOnChainLoaded = prev; // restore the canonical handler
      if (typeof prev === 'function') prev();
      restore('&#8635; REFRESH');
    };
    setTimeout(function () {
      if (done) return;
      window.itpOnChainLoaded = prev;
      restore('&#8635; REFRESH');
      if (!window.itOptionChainData) {
        alertModal('Chain load failed', 'Could not load the option chain (network or token issue). Try again.');
      }
    }, 8000);
    try { window.itFetchChain(true); } catch (_) {
      window.itpOnChainLoaded = prev;
      restore('&#8635; REFRESH');
    }
  }
  ...
  // Expose API
  window.itpBuy = function () { place('LONG'); };
  window.itpSell = function () { place('SHORT'); };
  window.itpBuyCE = function () { placeOpt('CE'); };
  window.itpBuyPE = function () { placeOpt('PE'); };
  window.itpExit = function (id) { exit(id, 'MANUAL'); };
  window.itpExitAll = exitAll;
  window.itpReset = reset;
  window.itpClearHistory = clearHistory;
  ...
  window.itpIncQty = incQty;
  window.itpSetQty = setQty;
  // Order-ticket form handlers (LIMIT / STOP)
  window.itpSetOrderType = setOrderType;
  window.itpSetTriggerPrice = setTriggerPrice;
  window.itpSetAttachedRisk = setAttachedRisk;
  window.itpCancelPending = cancelPending;
  window.itpCancelAllPending = function () { cancelAllPending(false); };
  window.itpLoadStrikes = loadStrikes;
  window.itpOnChainLoaded = onChainLoaded;
  window.itpToggleStrikes = toggleStrikes;
  window.itpStrikeSearch = strikeSearch;
  window.itpUpdateLastFetch = updateLastFetch;
  window.itpUpdatePosRisk = updatePosRisk;
  // ── Spot-trigger prime (May 2026 spot-first refactor) ────────
  // paperBridgeModule calls this BEFORE invoking itpBuyCE / itpBuyPE
  ...
  window.itpPrimeSpotTriggers = function (optType, slSpot, tgtSpot, entrySpot) {
    if (!optType) return;
  ...
  // module state nor change intradayPaperTradeModule's behaviour, just
  // surface the read/setter the bridge needs.
  //
  //   itpSelectStrike(k)   — same as a manual strike-picker click.
  //                         Sets state.selectedStrike, runs the
  //                         normal apply + poll + analyzer-notify
  //                         pipeline. Idempotent.
  //   itpGetPosition(id)   — read-only snapshot of an open OR
  ...
  window.itpSelectStrike = selectStrike;
  window.itpGetPosition = function (id) {
  ...
  // Live Nifty 50 spot LTP — last value pushed via itpPaperTradeTick.
  ...
  window.itpPaperTradeGetLastSpot = function () {
    var v = state.lastLTP;
  ...
  // a strike manually.
  window.itpPaperTradeKickOptionPolling = function () {
    if (shouldPollOptions()) {
      if (!state.optPollTimer) startOptionPolling();
  ...
  // planning to trade.
  window.itpPaperTradeGetSelectedStrike = function () {
    return (state.selectedStrike != null) ? +state.selectedStrike : null;
  };
  window.itpPaperTradeGetOptionPrice = function (instrumentKey) {
    if (!instrumentKey) return null;
    var px = state.optPrices[instrumentKey];
    return (isFinite(px) && px > 0) ? px : null;
  };

  // NOTE: the cloned-from-Options "API pause banner" (renderPtApiPauseBanner +
  // itpToggleApiPause) was REMOVED. It targeted #pt-api-pause-banner (an
  // Options-tab id that doesn't exist in content/intraday-trade.html) and its
  // toggle poked window.tvState / window.tvReload (the Options/Live chart) —
  // exactly the cross-tab wiring we're eliminating. The intraday tab has its
  // OWN pause banner (#it-api-pause-banner) + toggle (window.itToggleApiPause),
  // owned by scripts/intraday-trade.js. This clone simply DEFERS to that single
  // switch via itpIsApiPaused() → window.itIsApiPaused().

  // ── Intraday-tab lifecycle hook ──────────────────────────────────────────
  // Called by scripts/intraday-trade.js activate() AFTER content/intraday-trade.html
  // has been injected (the DOMContentLoaded renderAll below no-ops because the
  // itp-* DOM doesn't exist at module-eval time). It: hydrates the durable
  // server book, re-renders, pulls the option chain so the strike picker fills,
  // and kicks the option-LTP poller if there's anything to price.
  window.itpHydrate = hydrateFromServer;
  window.itpActivate = function () {
    renderAll();
    hydrateFromServer();
    // Fill the strike picker using the intraday tab's OWN chain fetch
    // (window.itFetchChain, owned by scripts/intraday-trade.js). On success it
    // sets window.itOptionChainData and notifies window.itpOnChainLoaded. No
    // dependency on the Options tab's chain module.
    try {
      if (!itpIsApiPaused()) {
        if (window.itOptionChainData) onChainLoaded();
        else if (typeof window.itFetchChain === 'function') window.itFetchChain();
      }
    } catch (_) {}
    try { if (shouldPollOptions()) { startOptionPolling(); pollOptionPrices(); } } catch (_) {}
  };

  // Guarded test hook — publishes the closure-private state + persistence so a
  // Node sandbox (window.__ITP_TEST__ set before load) can verify the server-JSON
  // save/load round-trip. No-op in the browser; nothing leaks.
  if (typeof window !== 'undefined' && window.__ITP_TEST__) {
    window.__itpExports = {
      getState: function () { return state; },
      serverPayload: serverPayload,
      scheduleServerSave: scheduleServerSave,
      hydrateFromServer: hydrateFromServer,
      resetServerHydrate: function () { serverHydrated = false; }
    };
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function () { load(); renderAll(); bindGlobalPickerHandlers(); });
  } else {
    load();
    renderAll();
