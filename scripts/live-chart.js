...
(function liveChartModule() {
  ...
  // ═══════════════════════════════════════════════════════════════════
  // SHARED UPSTOX RATE-LIMIT GATE  (lives on window so both pollers
  // — the chart's spot pollTick AND paper-trade's option pollOptionPrices
  // — share a single back-off state).
  // ═══════════════════════════════════════════════════════════════════
  ...
  window._upstoxNote429 = function (source) {
    ...
    // mounted (e.g. before initial chart render).
    try {
      setStatus('throttled', 'RATE LIMITED \u00B7 ' + sec + 's');
      ...
      if (!window._upstoxThrottleTicker) {
        window._upstoxThrottleTicker = setInterval(function () {
          ...
          if (rem > 0) {
            setStatus('throttled', 'RATE LIMITED \u00B7 ' + rem + 's');
          } else {
    ...
    } catch (_) { /* setStatus not available yet — non-fatal */ }
    return sec;
  };
